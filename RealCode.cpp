...
int main()
{
    ...
    string inputFilename = "C:\\Users\\HP\\Desktop\\CodeMaria\\fileOutput1.txt"; // Path to the input file
    string outputFilename = "output.txt"; // Path to the output file
    int choice;

    // Prompt the user to choose an action
    cout << "Choose an option:" << endl;
    cout << "1. Read from an existing file (" << inputFilename << ")" << endl;
    cout << "2. Enter new data into the output file (" << outputFilename << ")" << endl;
    cout << "Enter your choice (1 or 2): ";
    cin >> choice;
    cin.ignore(); // Clear the input buffer to avoid issues with getline()

    // If the user chooses to read the file
    if (choice == 1)
    {
